...
const char* WIFI_SSID = "Ubiquiti_LEICI"; // Fibertel WiFi565 2.4GHz        Ubiquiti_LEICI
...
void readAnalogSignal() {
  ...
  orpValues[readIndex] = voltage; // Usamos 'voltage' en lugar de 'orpValue'
  ...
  if (numReadings >= 10) {
    ...
    Point valor_crudo_ORP("valor_crudo_ORP"); // Cambio el nombre del punto
    valor_crudo_ORP.addTag("device", DEVICE);
    valor_crudo_ORP.addField("voltage_value", voltage); // Cambié orp_value por voltage_value
...
void loop() {
  ...
  if (shouldSendData) {
    ...
    if (client.validateConnection()) {
      ...
      Point valor_crudo_ORP("valor_crudo_ORP"); // Crear el punto de datos aquí
      valor_crudo_ORP.addTag("device", DEVICE);
      valor_crudo_ORP.addField("voltage_value", voltage); // Agrega el valor de voltage

      if (!client.writePoint(valor_crudo_ORP)) {
        ...
        Serial.println(voltage); // Imprimir el valor de voltage
        Serial.print("Tag: device = ");
        Serial.println(DEVICE); // Imprimir el tag
        Serial.print("Field: voltage_value = ");
        Serial.println(voltage); // Imprimir el valor del campo
      }
    } else {
      Serial.println("Error de conexión a InfluxDB.");
    }

    // Restablecer la bandera para enviar datos
